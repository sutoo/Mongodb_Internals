...
Secondaries还会把自己的同步进度上报给它的 sync source，这使得primary得以实现 write concerns功能。
因为primary需要知道写操作被同步到了多少secondaries上，才可以根据不同的write concerns，来选择是否返回写入成功。
...
如何选择一个 sync source候选人：
1 Secondary本地的replica set cache保存着自己知道的最新的primary的OpTime，可以成功候选人的条件为：
  对面的最新的oplog的entry的时间，不能比primary的OpTime小`maxSyncSourceLagSecs`秒
2 接着，选择latency最小的
3 如果没有满足的候选人，等1秒，再重新选择。
...
下面是查看rs.status()中的members中，包含的Optimes字段
```
"members" : [
          {
                  "_id" : 0,
